Feature: Bookcategory
  ...
  As a user I would like to check bookcategory page
...
Example:
  | filter      |  List                                                                   |
  | sort by     | Name: A to Z : Z to A, Price: Low to High : High to Low, Created on     |
  | Display     | 3,6,9                                                                   |
...
Then    I should be able to choose any filter op[tion from the list
...
Examples:
  ...
  | sort by    | Price: Low to High | sorted product with the price ascending order                      |
  | sort by    | Price: High to Low | sorted product with the price desending order                      |
...
Then     I should be able to see product display format according display format type
         as per given picture in srs document
